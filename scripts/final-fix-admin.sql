...
-- Temporarily drop the constraint to fix the mismatch
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_status_check;

-- Re-add the constraint with 'active' included if it was missing (it should be there, but let's be safe)
-- The error says the row contains 'active' but it violates the check, which implies 'active' is not in the allowed list
...
BEGIN
    ...
    -- Note: We're setting status to 'active' which now matches the constraint above
